...
export const useAuth = () => {
  ...
  return {
    ...
    token,
    isAuthenticated,
    isAdmin: typeof user?.role === 'string' ? user?.role === 'admin' : (user?.role as any)?.name === 'admin',
    isSpecialist: typeof user?.role === 'string' ? user?.role === 'specialist' : (user?.role as any)?.name === 'specialist',
    error,
    isLoading,
    login,
    register,
